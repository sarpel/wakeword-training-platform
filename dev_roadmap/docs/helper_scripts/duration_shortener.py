...
class AudioDurationShortener:
    ...
    def process_file(self, file_path: Path) -> int:
        ...
        # Split into chunks
        chunks = self._split_audio(audio, sr)
    ...
    def get_statistics(self, directory: Path, recursive: bool = True) -> dict:
        """
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help='Minimum duration for last chunk (default: 2.0)'
    )
    parser.add_argument(
        '--sample-rate',
        type=int,
    ...
    if stats['files_needing_split'] > 0:
        avg_long_duration = np.mean([
            d for d in [stats['avg_duration']]
            if d > args.duration
        ]) if stats['avg_duration'] > args.duration else stats['max_duration']
    ...
    if stats['files_needing_split'] > 0:
        ...
        files_processed, total_chunks = shortener.process_directory(
            directory,
