...
def main():
    ...
    default_output = project_root / "data" / "trimmed_dataset"

    parser = argparse.ArgumentParser(description="Trim silence from audio dataset")
    parser.add_argument("--dataset", type=str, default=str(default_dataset), help="Path to dataset root")
    parser.add_argument(
        "--output", type=str, default=str(default_output), help="Output directory (default: data/trimmed_dataset)"
    )
    ...
    count = scanner.trim_dataset_silence(output_dir=args.output, top_db=args.top_db, overwrite=args.overwrite)
