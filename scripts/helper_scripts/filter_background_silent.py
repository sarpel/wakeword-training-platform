...
def parse_args():
    ...
    parser.add_argument("--background_dir", type=str,
                       default="data/raw/background", 
                       help="Target background directory")
    parser.add_argument("--min_rms_db", type=float, default=-54.0,
                       help="Minimum RMS dB to restore files (default: -54.0)")
...
def main():
    ...
    background_silent = df[
        (df['status'] == 'silent') & 
        (df['path'].str.contains('background')) &
        (df['rms_dbfs'] >= args.min_rms_db)
    ]
