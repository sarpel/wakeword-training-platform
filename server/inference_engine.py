...
class InferenceEngine:
    def __init__(self, model_path: Optional[str] = None, device: str = "cpu"):
        ...
        self.audio_processor = AudioProcessor(
            config=self.config, cmvn_path=paths.CMVN_STATS if paths.CMVN_STATS.exists() else None, device=self.device
        )
    ...
    def _load_model(self, model_path: Optional[str]):
        if not model_path or not os.path.exists(model_path):
            ...
            fallback_path = paths.CHECKPOINTS / "best_model.pt"
            if not fallback_path.exists():
                raise FileNotFoundError(f"Model not found at {fallback_path}")
        ...
        feature_dim = (
            config.data.n_mels if config.data.feature_type in ["mel", "mel_spectrogram"] else config.data.n_mfcc
        )
