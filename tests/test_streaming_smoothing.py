...
def test_n_of_m_smoothing_activation():
    ...
    # N=3, M=5
    detector = StreamingDetector(threshold_on=0.5, vote_window=5, vote_threshold=3, lockout_ms=0)
...
def test_n_of_m_smoothing_sliding():
    ...
    # N=2, M=3
    detector = StreamingDetector(threshold_on=0.5, vote_window=3, vote_threshold=2, lockout_ms=0)
...
def test_lockout_prevents_retrigger():
    """Test lockout logic."""
    detector = StreamingDetector(threshold_on=0.5, vote_window=1, vote_threshold=1, lockout_ms=1000)  # Immediate
