...
def test_mobilenetv3_hybrid_config():
    ...
    model1 = create_model(architecture="mobilenetv3", num_classes=2, pretrained=False, dropout=0.5, input_channels=1)
...
def test_tinyconv_dynamic_channels():
    ...
    model1 = create_model(architecture="tiny_conv", num_classes=2, input_channels=1, dropout=0.3)
    print("[OK] TinyConv with default channels created")

    # Test 2: Custom channel configuration (small)
    model2 = create_model(
        architecture="tiny_conv", num_classes=2, input_channels=1, dropout=0.2, tcn_num_channels=[8, 16, 32]
    )
...
def test_tcn_config_parameters():
    ...
    # Test 1: Default TCN
    model1 = create_model(architecture="tcn", num_classes=2, input_size=40)
...
def test_cddnn_config_parameters():
    ...
    model1 = create_model(architecture="cd_dnn", num_classes=2, input_size=2000)  # 40 features * 50 frames
...
def test_lstm_gru_config():
    ...
    gru_model = create_model(
        architecture="gru", num_classes=2, input_size=60, hidden_size=128, num_layers=1, bidirectional=True, dropout=0.2
    )
