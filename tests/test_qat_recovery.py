...
from src.training.qat_utils import prepare_model_for_qat, convert_model_to_quantized
...
class TestQATRecovery:
    ...
    def test_quantization_error_report(self, model, qat_config):
        ...
        val_data = [
            (torch.randn(1, 1, 4, 4), torch.tensor([0])),
            (torch.randn(1, 1, 4, 4), torch.tensor([1]))
        ]
