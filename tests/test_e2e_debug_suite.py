...
def test_full_debug_workflow(mock_load, mock_eval_state):
    ...
    eval_state.test_results = [
        EvaluationResult(filename="file1.wav", prediction="Positive", confidence=0.9, label=0, raw_audio=np.zeros(16000), logits=np.array([0.9]), latency_ms=10.0), # FP
        EvaluationResult(filename="file2.wav", prediction="Negative", confidence=0.1, label=0, raw_audio=np.zeros(16000), logits=np.array([0.1]), latency_ms=10.0)  # TN
    ]
