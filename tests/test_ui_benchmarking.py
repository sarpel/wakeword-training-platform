...
import torch
import numpy as np
from src.ui.panel_evaluation import run_benchmark_test, eval_state
...
def test_run_benchmark_success(mock_stage, mock_runner):
    ...
    eval_state.model_info = {
        "config": MagicMock(
            model=MagicMock(architecture="mobilenetv3"),
            data=MagicMock(sample_rate=16000, audio_duration=1.0)
        )
    }
