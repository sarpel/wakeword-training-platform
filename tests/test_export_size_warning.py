...
from src.config.defaults import SizeTargetConfig, get_default_config
...
class TestExportSizeWarning:
    ...
    def test_size_warning_trigger(self, mock_exporter_cls, mock_create_model, mock_torch_load, tmp_path):
        ...
        mock_checkpoint = {
            "config": config,
            "model_state_dict": {"fc.weight": torch.randn(2, 10), "fc.bias": torch.randn(2)},
        }
        ...
        results = export_model_to_onnx(checkpoint_path=Path("dummy.pt"), output_path=output_path, device="cpu")
