...
from src.export.onnx_exporter import ExportConfig, ONNXExporter, export_model_to_onnx
...
def test_tflite_export_flow():
    ...
    # 3. Save checkpoint
    torch.save(
        {"model_state_dict": model.state_dict(), "config": config.to_dict(), "epoch": 0, "val_loss": 0.0},
        checkpoint_path,
    )
    ...
    try:
        results = export_model_to_onnx(
            checkpoint_path=checkpoint_path, output_path=onnx_path, export_tflite=True, device="cpu"
        )
