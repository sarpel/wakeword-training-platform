...
class TestLossFunctions:
    ...
    def test_create_focal_loss(self, device):
        ...
        loss_fn = create_loss_function("focal", num_classes=2, focal_alpha=0.25, focal_gamma=2.0, device=device)
    ...
    def test_label_smoothing_loss(self, device):
        ...
        loss_fn = create_loss_function("cross_entropy", num_classes=2, label_smoothing=0.1, device=device)
    ...
    def test_focal_loss_reduces_easy_examples(self, device):
        ...
        focal_loss = create_loss_function("focal", num_classes=2, focal_gamma=2.0, device=device)
    ...
    def test_loss_gradient_flow(self, device):
        ...
        predictions = torch.randn(4, 2, device=device, requires_grad=True)
        targets = torch.tensor([0, 1, 0, 1]).to(device)
