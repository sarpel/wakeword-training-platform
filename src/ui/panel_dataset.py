    ...
    with gr.Blocks() as panel:
        ...
        def scan_datasets_handler(
            ...
            try:
                ...
                # Validate root path
                root_path_obj = validate_path(root_path, must_exist=True, must_be_dir=True)

                logger.info(f"Scanning datasets in: {root_path_obj} (skip_validation={skip_val})")
        ...
        def split_datasets_handler(
            ...
            try:
                ...
                if npy_source:
                    npy_source_path = validate_path(npy_source, must_exist=True, must_be_dir=True)
                else:
                    # Default fallback relative to data root
                    npy_source_path = validate_path(Path(root_path) / "npy")
                ...
                output_dir = validate_path(Path(root_path).parent / "splits")
        ...
        def analyze_npy_handler(npy_path: str, root_path: str, progress: gr.Progress = gr.Progress()) -> str:
            ...
            try:
                ...
                if not npy_path:
                    ...
                    npy_path_obj = validate_path(Path(root_path) / "npy", must_exist=True, must_be_dir=True)
                else:
                    npy_path_obj = validate_path(npy_path, must_exist=True, must_be_dir=True)
        ...
        def validate_shapes_handler(
            ...
            try:
                # Determine npy folder path and validate
                if not npy_path:
                    if not root_path:
                        return "❌ Please provide either .npy folder path or scan datasets first"
                    npy_path_obj = validate_path(Path(root_path) / "npy", must_exist=True, must_be_dir=True)
        ...
        def vad_filter_handler(
            ...
            try:
                ...
                # Validate root path and construct manifest path
                validated_root = validate_path(root_path, must_exist=True, must_be_dir=True)
                manifest_path = validated_root.parent / "splits" / "dataset_manifest.json"
